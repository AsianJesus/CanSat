						   ...
						   XMMATRIX baseViewMatrix)
{
	...
	result = InitializeSentence(&m_sentence10, 50, device);
	if(!result)
	{
		return false;
	}
...
bool TextClass::SetSessionInfo(int id, char* name, time_t startTime, ID3D11DeviceContext* deviceContext)
{
	...
	result = UpdateSentence(m_sentence8, (char*)temp.c_str(), 10, 210, 0.0f, 1.0f, 0.0f, deviceContext);
	if(!result)
	{
		return false;
	}
	...
	result = UpdateSentence(m_sentence9, (char*)temp.c_str(), 10, 230, 0.0f, 1.0f, 0.0f, deviceContext);
	if(!result)
	{
		return false;
	}
	...
	temp = "Session started at: " + std::string(buffer);
	if (temp.size() > 49) temp = temp.substr(0, 49);
	result = UpdateSentence(m_sentence10, (char*)temp.c_str(), 10, 250, 0.0f, 1.0f, 0.0f, deviceContext);
	if(!result)
	{
		return false;
	}
