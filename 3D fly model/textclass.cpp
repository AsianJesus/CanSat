						   ...
						   XMMATRIX baseViewMatrix)
{
	...
	result = InitializeSentence(&m_sentence8, 18, device);
	if(!result)
	{
		return false;
	}

	// Initialize the ninth sentence. 30
	result = InitializeSentence(&m_sentence9, 30, device);
	if(!result)
	{
		return false;
	}
	...
	result = InitializeSentence(&m_sentence10, 36, device);
	if(!result)
	{
		return false;
	}
...
bool TextClass::RenderSentence(SentenceType* sentence, ID3D11DeviceContext* deviceContext, FontShaderClass* FontShader, XMMATRIX worldMatrix, 
...
bool TextClass::SetVideoCardInfo(char* videoCardName, int videoCardMemory, ID3D11DeviceContext* deviceContext)
{
	...
	result = UpdateSentence(m_sentence1, dataString, 10, 10, 0.0f, 0.0f, 0.0f, deviceContext);
	if(!result)
	{
		return false;
	}
	...
	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence2, memoryString, 10, 30, 0.0f, 0.0f, 0.0f, deviceContext);
	if(!result)
	{
		return false;
	}
...
bool TextClass::SetFps(int fps, ID3D11DeviceContext* deviceContext)
{
	...
	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence3, fpsString, 10, 70, 0.0f, 0.0f, 0.0f, deviceContext);
	if(!result)
	{
		return false;
	}
...
bool TextClass::SetCpu(int cpu, ID3D11DeviceContext* deviceContext)
{
	...
	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence4, cpuString, 10, 90, 0.0f, 0.0f, 0.0f, deviceContext);
	if(!result)
	{
		return false;
	}
...
bool TextClass::SetCameraPosition(float posX, float posY, float posZ, ID3D11DeviceContext* deviceContext)
{
	...
	result = UpdateSentence(m_sentence5, dataString, 10, 130, 0.0f, 0.0f, 0.0f, deviceContext);
	if(!result)
	{
		return false;
	}
	...
	result = UpdateSentence(m_sentence6, dataString, 10, 150, 0.0f, 0.0f, 0.0f, deviceContext);
	if(!result)
	{
		return false;
	}
	...
	result = UpdateSentence(m_sentence7, dataString, 10, 170, 0.0f, 0.0f, 0.0f, deviceContext);
	if(!result)
	{
		return false;
	}
...
bool TextClass::SetSessionInfo(int id, char* name, time_t startTime, ID3D11DeviceContext* deviceContext)
{
	...
	string temp = "Session ID: " + std::to_string(id);
	if (temp.size() > 19) temp = temp.substr(0, 19);
	result = UpdateSentence(m_sentence8, (char*)temp.c_str(), 10, 210, 0.0f, 0.0f, 0.0f, deviceContext);
	if(!result)
	{
		return false;
	}

	temp = "Session name: " + string(name);
	if (temp.size() > 29) temp = temp.substr(0, 29);
	result = UpdateSentence(m_sentence9, (char*)temp.c_str(), 10, 230, 0.0f, 0.0f, 0.0f, deviceContext);
	if(!result)
	{
		return false;
	}
	char* buffer = new char[64];
	ctime_s(buffer, 64, &startTime);
	string t = std::string(buffer);
	temp = "Session started at: " + t.substr(4, t.size() - 10);
	if (temp.size() > 36) temp = temp.substr(0, 35);
	result = UpdateSentence(m_sentence10, (char*)temp.c_str(), 10, 250, 0.0f, 0.0f, 0.0f, deviceContext);
	if(!result)
	{
		return false;
	}
